---
description: "신학 학술 연구 및 논문 서베이 에이전트 가동 (theology-research)"
---

# 🌐 Agent Action Spec: `/theology-research` (Mosaic v2.0)

1. **[QUERY EXPANSION]**: 입력된 한국어 주제를 바탕으로 학술적 영어(EN) 및 독일어(DE) 전문 용어 세트를 생성한다. (Theology-Translator 로직 활용)
...
3. **[FORENSIC GATE]**: **의무 사항**. 모든 소스의 신학적 관련성을 감사하고 DOI/URL 실존 여부를 검증한다. 할루시네이션 및 주제 불일치 항목(노이즈)을 원천 차단한다.
4. **[PAF & APORETICS]**: Cathedral Engine of 1차 텍스트 분석 및 `MS_Brain.nosync` 지역 규칙(TRE 용어 등)을 적용한다.
5. **[OUTPUT: MOSAIC SCHEMA]**:
    - 본문은 신학적 인사이트와 변증적 TDD 중심으로 서술한다.
