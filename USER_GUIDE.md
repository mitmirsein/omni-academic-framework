...
| Review | 5인 에이전트 패널 비평 및 최종 편집장 평결 | `review.json`, `review.md` |
...
`--module review`는 생성된 초안(`draft.json`)을 바탕으로 5인 에이전트 패널의 비평 및 심층 리뷰 과정을 수행한다. 
...
3. **산출물**:
   - `review.json`(구조화된 5인 비평 및 평결)과 `review.md`(가독성 있는 비평 리포트)가 저장된다.
...
- `--module review` 실행 시 `review.json`, `review.md` 상대 링크 및 5인 패널 평결 내용 반영
